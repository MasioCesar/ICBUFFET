...
export const AccountProfileDetails = ({ userDetails }) => {
    ...
    const { cancelReservation, cancelRestaurantReservation } = useContext(GetContext);
    ...
    const handleConfirmCancel = () => {
        ...
        const reservation = comandas[index];

        const restaurantId = reservation.restaurantId; // Certifique-se de que o `restaurantId` está nos dados
        const tableUid = reservation.tableUid; // Certifique-se de que o `tableUid` está nos dados

        // Remove a reserva do Firestore
        cancelRestaurantReservation(restaurantId, tableUid, index);
