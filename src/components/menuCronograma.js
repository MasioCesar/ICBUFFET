...
export const MenuCronograma = ({ addOrder }) => {
		...
		if (storedRestaurantId) {
			const fetchRestaurants = async () => {
				try {
					...
					// Garante que os pratos estejam disponíveis e são um array
					if (Array.isArray(data.cardapio)) {
						setCardapio(data.cardapio);
					} else {
						console.warn('Cardápio inválido ou não encontrado');
					}
				} catch (error) {
	...
	// Filtro para exibir apenas os pratos da categoria selecionada
	const pratosFiltrados =
		categoriaAtiva === 'Todos'
			? pratos
			: pratos.filter((prato) => prato.tipo === categoriaAtiva);
						...
						<div className="relative lg:hidden">
							...
							{dropdownOpen && (
								<div className="absolute top-full right-0 mt-2 bg-[#390912] border-2 border-[#F3DFA0] rounded-lg shadow-lg">
									{['Todos', 'Entrada', 'Principal', 'Bebida'].map((category) => (
										<button
											key={category}
											onClick={() => {
											...
											className={`w-full text-center px-4 py-2 hover:bg-[#F3DFA0] hover:text-[#000000] ${
						...
						<div className="hidden lg:flex px-6 justify-center gap-4">
							{['Todos', 'Entrada', 'Principal', 'Bebida'].map((category) => (
								<button
									...
									className={`w-[86px] h-[34px] rounded-2xl font-roboto text-[14px] lg:text-[16px] transition-colors border-2 ${
										categoriaAtiva === category
				...
				{pratosFiltrados.map((prato, index) => (
					...
					>
						<div className="relative sm:w-[170px] sm:h-[170px] w-[120px] h-[120px] flex-shrink-0 rounded-md overflow-hidden shadow-lg">
							<Image
								src={prato.imagem ? prato.imagem : '/menu.png'}
								alt={prato.alt || 'Imagem do prato'}
								fill
						...
						<div className="text-left flex-1">
							<h2 className="text-[#ffffff] text-[20px] md:text-[24px] font-bebas-neue">
								{prato.nome}
							</h2>
							...
							<p className="text-[#20170E] text-[20px] font-bebas-neue bg-[#F3DFA0] py-0.5 px-2 rounded-md inline-block">
								<span className="text-[14px] font-normal mr-0.5">
									{prato.preco.slice(0, 3)}
								</span>
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
					<div className="relative bg-white p-4 rounded-lg shadow-lg w-11/12 sm:w-3/4 md:w-1/2 lg:w-1/3">
						<div className="flex items-center justify-between mb-2">
							<h3 className="text-black text-2xl pt-1 font-poppins font-bold">
								{selectedPrato}
							</h3>
							<button
